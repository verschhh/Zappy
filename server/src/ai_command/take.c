...
void switch_case_take(int i, map_t *map_cpy, client_t *client_cpy, int sockfd)
{
    switch (i) {
        case 0:
            if (check_element_tile(map_cpy->food, sockfd) == 1) {// check if there is enough food / warning
                map_cpy->food--;
                client_cpy->player->inventory->food++; //warning
            }
            break;
        case 1:
            if (check_element_tile(map_cpy->linemate, sockfd) == 1) {// check if there is enough linemate / warning
                map_cpy->linemate--;
                client_cpy->player->inventory->linemate++; //warning
            }
            break;
        case 2:
            if (check_element_tile(map_cpy->deraumere, sockfd) == 1) {// check if there is enough deraumere / warning
                map_cpy->deraumere--;
                client_cpy->player->inventory->deraumere++; //warning
            }
            break;
        default:
